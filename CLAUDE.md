...
- **目前 Stage**：Stage 3 已完成並部署（Cloud Run，服務網址 https://shortlink-ljrbbufbfq-de.a.run.app ，Cloud SQL Postgres `asia-east1`/`db-f1-micro`、secrets 走 Secret Manager）。之後又做了一批 **Stage 3 之後的登入/UI 打磨（已在本機真人驗收，並已部署到 Cloud Run：revision `shortlink-00005`；線上 `/livez` 與 entrance 頁已驗證，提醒對外只用新格式網址 https://shortlink-ljrbbufbfq-de.a.run.app ，OAuth redirect URI 只設在這組）**：(1) 介面全英文 + 可切換 dark mode（手動切、記 localStorage）+ 按鈕 hover 改成陰影/變色（不上浮）；(2) **OpenAI 風格 email-first 登入入口**（`apps/accounts`，`/accounts/start/`）：填 email → 已註冊轉登入、未註冊轉註冊、純社群帳號轉「Continue with Google/Facebook」引導頁；(3) **跨 provider 自動連結**（同已驗證 email 用第二個 provider 登入會連到同一帳號；Facebook 設 `VERIFIED_EMAIL: True`）；(4) 修掉登入失敗時 non-field error 被吃掉、畫面無反應的 bug。決策見 [`docs/adr/0009-email-first-auth.md`](docs/adr/0009-email-first-auth.md)。**再來又做了一批登入/UX 打磨（items 5–8，現已連同下面的 items 9–10 一起部署上 Cloud Run 並真人驗收通過）**：(5) 社群登入按鈕改成 **POST**，跳過 allauth 的「Continue」中間確認頁（保留 CSRF 保護，而非關掉 `SOCIALACCOUNT_LOGIN_ON_GET`）；(6) signup 拿掉常駐的密碼規則 help text（4 個 `AUTH_PASSWORD_VALIDATORS` 照常作用，違規才出紅字）；(7) 移除登入頁「Forgot your password?」連結——無寄信基礎設施、密碼重設無法運作，且 app 是 social-login-first，移除入口好過保留會 500 的功能（reset URL 本身仍在，必要時可再擋）；(8) dev 改用 console email backend（忘記密碼不再 ConnectionRefused 500）。(5)(7) 透過 `CustomSignupForm`/`CustomLoginForm`（`ACCOUNT_FORMS`）實作；過程順手修掉一個回歸 bug：多行 `{# #}` 註解（單行限定）外漏成頁面文字，改用 `{% comment %}`。**最新一批（items 9–10，已 commit/push，且已部署上 Cloud Run、真人驗收通過：Google 登入確認會跳「選擇帳號」、線上直接打 `accounts/password/reset/` 回 404）**：(9) **Google 登入每次強制跳「選擇帳號」**——在 `SOCIALACCOUNT_PROVIDERS["google"]` 加 `"AUTH_PARAMS": {"prompt": "select_account"}`（本機真人驗收：即使只有單一帳號也會先跳出選帳號頁；`prompt=select_account` 為 Google 專屬，Facebook 無等價參數，其 `auth_type=reauthenticate` 是「重輸密碼」而非「換帳號」，故 FB 換帳號仍只能靠無痕視窗）；(10) **prod 擋掉密碼重設頁**——新增 `PASSWORD_RESET_ENABLED` 開關（`base.py`=True、`prod.py`=False、dev 繼承 base 的 True），在 `config/urls.py` 的 allauth `include` 之前、當開關關閉時用一個 `raise Http404` 的小 view（`_disabled`）覆寫掉重設入口 `accounts/password/reset/` 與信件 key 確認頁 `accounts/password/reset/key/...`（兩個 `done` 純文字頁不擋，反正入口被擋就走不到）；dev 維持可用（已用 Django test client 分別驗證 dev=200、prod=404）。**Facebook 無法由 app 端強制「選帳號」（平台限制，靠無痕視窗）。** **接著又做了一批前台 UI/UX 改造（首頁與 My Links 儀表板，已部署到 Cloud Run 並真人驗收通過：線上 `/livez` 回 ok、首頁新版登入卡與儀表板皆已上線）**：(a) 首頁改成「看登入狀態的單一入口」——未登入直接嵌入 email-first 登入（重用 `account_entrance` 表單 + allauth 社群 POST 片段），已登入則 `redirect` 到儀表板；nav 拿掉多餘的 My Links / Create / Sign In 連結（品牌字回首頁即可到達）。(b) **My Links 變成單頁儀表板**：「+ Create」改用瀏覽器原生 `<dialog>` modal 建立（POST 回 `my_links`、成功走 PRG 重導、失敗自動重開並顯示錯誤），獨立的 Create 頁／URL／view 整組移除；`my_links` view 改成同時處理 GET 顯示與 POST 建立。(c) **整張卡點擊即複製短網址**（`navigator.clipboard.writeText` + 底部 toast 回饋；短碼藍字 `target="_blank"` 開新分頁，並用 `event.stopPropagation()` 讓點藍字不誤觸複製）。(d) **點擊明細可就地展開**：預設露 3 筆，>3 筆才在區塊右上顯示扁平 SVG chevron，點擊以 inline accordion 切換「3 筆預覽 ↔ 全部點擊（`max-h-72` 可捲動）」並轉動箭頭；preview/full 都套自訂細捲軸（`::-webkit-scrollbar` + `scrollbar-width`）並用 `scrollbar-gutter: stable` 預留捲軸空間，避免展開時表格跑位。過程踩到的坑：多行 `{# #}` 註解外漏成頁面文字（同 items 5–8 的老坑，且因漏出的字裡含 `<dialog>` 還連帶造成隱形 dialog 吞掉內容，改用 `{% comment %}`）、可展開明細若放進「可點即複製」的卡片內會因事件冒泡誤觸複製（改放卡外或加 `stopPropagation`）。chevron 必須放在「包住 preview/full 的 `relative` 容器」上、而非放進會互相隱藏的 preview/full（否則展開後 chevron 跟著消失）。(e) 首頁登入卡版面對齊 entrance 頁（`max-w-sm` 窄卡、`{% comment %}element h1{% endcomment %}` 大標題、`rounded-2xl p-8`），但外層 hero 維持 `max-w-md` 讓副標題保持一行。**這批部署時踩到三個雷，已寫進 README 部署章節的「重新部署」runbook：① build 要在專案目錄（含 Dockerfile）跑，否則 `Dockerfile required`；② image path 的專案是 `shortlink-499808`（README 舊版誤寫 `shortlink-demo`，已全數修正），用錯會 push `denied`；③ 重新部署只給 `--image`，千萬別帶 `--set-secrets`（整組覆蓋，漏 `REDIS_URL` 會洗掉線上設定、`prod.py` 缺它即啟動報錯）。** **再來又做了一小批儀表板功能（已實作、本機自動化驗證通過，待 commit + 部署）**：(f) **刪除短網址**——新增 owner-scoped 的 `delete_link` view（`get_object_or_404(ShortLink, pk=pk, owner=request.user)`：找不到或非本人一律 404，杜絕刪別人的連結）+ POST-only（GET 不刪）+ PRG 導回 `my_links`；URL `links/<int:pk>/delete`（放在萬用短碼路由之上）；卡片右上垃圾桶鈕用 CSRF 的 POST form，`onclick="event.stopPropagation()"` 避免誤觸整卡複製、`onsubmit="return confirm(...)"` 防手滑；刪除靠既有 `post_delete` 訊號自動清 Redis 快取。已用 rollback 交易測過：別的 user 刪→404 且連結還在、本人刪→302 且確實消失。(g) **列表新→舊排序**——`my_links` 查詢明確加 `.order_by("-created_at")`；踩到 Django 坑：`ShortLink.Meta.ordering=["-created_at"]` 在 view 用了 `.annotate(Count("clicks"))` 後會因 `GROUP BY` 失效（實測退回 id 升冪＝舊在前），所以要在查詢端明確 `order_by`，`annotate` 不會吃掉明確排序。 **接著又把儀表板再加兩個功能（已實作、本機自動化＋真人驗收通過，已 commit 並部署上線）**：(h) **多選項排序下拉**——把 (g) 的單一「新→舊」升級成可選排序：view 用白名單 dict `_SORT_OPTIONS` 把 `?sort=` query 參數對應到 `order_by` 參數（`newest`/`oldest`/`popular`/`title`；值為 tuple、用 `*` 解包並帶次要鍵 `-created_at` 讓相同值順序穩定；`title` 用 `Lower("title")` 大小寫不分；非白名單值一律落回 `newest`，避免使用者亂排任意欄位/跨關聯或打錯字 500），view 另算出正規化的 `current_sort` key 回傳給模板做 highlight（DB 與 UI 吃同一份驗證結果）。UI 用原生 `<details>`/`<summary>` 做「Sort by」下拉（零 JS，選項是 `?sort=` 連結、導頁即重排並自動收合；另加一段 document click 監聽讓「點外面」也能收合）。踩到的坑：單元素 tuple 漏結尾逗號 → `*` 解包字串逐字元 → `FieldError`/500（ruff 過不代表正確）。(i) **編輯短網址**——owner-scoped `edit_link` view（`get_object_or_404(pk, owner=request.user)`，POST 用 `ShortLinkForm(request.POST, instance=link)` 更新同一筆、PRG 回 `my_links`；`instance=` 讓 GET 預填、`.save()` 更新而非新增；改 `original_url` 靠既有 `post_save` 訊號自動清 Redis 快取——編輯免補任何快取碼，是當初把失效集中在訊號的回報）。UI 用每連結一個原生 `<dialog>` modal（與 Create 一致、預填現值），另保留獨立頁 `edit_link.html` 當「驗證失敗/直接打網址」的後備。踩到 `<dialog>` 置中坑：modal 必須跟 Create 放在**同一 DOM 層級**（放進卡片清單迴圈會因定位環境不同而上偏/右偏），最後用 `fixed inset-0 m-auto h-fit w-full max-w-lg` 兩軸置中、不靠瀏覽器預設。決策（可改目的地 URL 的取捨）見 [`docs/adr/0011-editable-destination-url.md`](docs/adr/0011-editable-destination-url.md)。 **(h)(i) 已連同下面的隱私權頁一起部署上線（revision `shortlink-00010-49m`；線上 `/livez`、`/privacy/` 皆回 200 驗證過）。** **接著為了「讓朋友/面試官也能登入」做了一批 OAuth 上線處理（多為 Google/FB 後台設定、非程式碼）**：(j) **隱私權政策 + 資料刪除頁**——`apps/core` 新增 `privacy` view、URL `/privacy/`、`templates/core/privacy.html`（含 `#data-deletion` 錨點），這是把 Facebook App 切「上線 (Live)」的硬門檻（FB 要求公開隱私權 URL 與資料刪除說明），URL 已填進 FB 後台 App settings→Basic。(k) **FB 上線卡關 → 改用測試者**：朋友從 **Messenger 內建瀏覽器**點連結時，Google 回 `403 disallowed_useragent`（Google「Use secure browsers」政策**禁止在 in-app WebView 跑 OAuth**，非本站 bug，須改用 Safari/Chrome 真瀏覽器）、FB 回「應用程式並非使用中」（App 在開發模式、只有角色能登入）。要把 FB 切 Live 時發現 **`email` 的 Advanced Access 硬性要求「商家驗證 (Business Verification)」**（操作選單只剩「前往商家驗證／移除」），對個人作品太重，故**改走「加測試者」**：App roles→角色→測試人員，加人時要用對方個人檔案網址裡的**使用者名稱**（純打中文名字會 `does not resolve to a valid user ID`），被加者需有免費 FB 開發者帳號並自行接受邀請。`public_profile` 自動授予、免 BV——當時以為「切 Live 但只留 public_profile」可日後對外全開又免 BV（後來證實此前提已不成立，見下）。 **接著加了網站 Logo（已部署 revision `shortlink-00011-qdw`：nav 品牌區 + 首頁 hero + favicon、靜態檔走 WhiteNoise hash 檔名）。** **最後處理「讓任何人都能登入」的對外開放決策並拍板（見 [`docs/adr/0012-facebook-public-profile-only.md`](docs/adr/0012-facebook-public-profile-only.md)）**：先嘗試「FB 只留 `public_profile`、丟 email」＋`SOCIALACCOUNT_EMAIL_REQUIRED=False`（讓無 email 也能無痛建帳號），想免 BV 切 Live；已部署 revision `shortlink-00012-572`、本機真人驗證「FB 同意畫面只剩姓名+大頭照、無 email 中間頁」。**但實測發現 Meta 現在「發佈 (go Live)」這步本身就硬性要求商家驗證，連 `public_profile` only 也擋**（發佈頁把 BV 列必要、移除 email use case 與解綁商家都沒用、發佈鈕仍灰），即丟 email 對「對外全開」毫無幫助、FB 怎樣都只能測試者用。**故退回 email + `public_profile` + 測試者**（還原跨 provider 連結亮點，反正 BV 卡死、留 email 純賺；程式碼退回 email-based，探索 branch 未併入 main 已捨棄、唯一保留 ADR 0012；線上 revision `shortlink-00013-7t8`），**並把「任何人都能登入」改由 Google 承擔：Google OAuth 同意畫面已發布成 In production**（`profile`+`email` 非敏感範圍、免驗證/免 BV，任何 Google 帳號皆可登入、無人數上限）。**待辦（FB 後台手動）：把先前從 use case 移除的 `email` 權限加回去，讓測試者登入仍取得 email。** **最後又做兩個首頁/登入 UI 微調並部署（revision `shortlink-00014-j8s`）：(1) 登入按鈕改成 Google 在上、Facebook 在下——覆寫 allauth `templates/socialaccount/snippets/provider_list.html`、迴圈加 `reversed`（因 allauth `get_providers` 預設 `sorted(key=p.name)`＝Facebook 字母在前）；(2) 移除首頁副標題「Shorten any URL, then track its clicks and source IPs.」讓 hero 乾淨。另為了「從零測試帳號建立/連結」，用「暫時授權本機 IP → Django `create_superuser` → 撤回授權」在 prod 建了一個 Django superuser（username `admin`、臨時密碼，已請作者登入後自行更改），再由作者透過 `/admin/` 刪光原有 5 個測試使用者（`CASCADE` 連帶清短網址/點擊/社群連結、`post_delete` 訊號清 Redis）。** **之後又把 social_only「Welcome back」continue 頁也排成 Google 在上（同 `reversed`，部署 revision `shortlink-00015-w5k`），並由作者把 `email` 加回 FB use case（測試者登入恢復取得 email）。期間釐清兩個已知限制（決定不修、僅記錄）：(i) FB 開發模式下角色登入時，同意畫面那條「送交登入審查／某些權限已不再獲得批准」紅字，是「權限只到『可供測試』(standard access)」的開發者提示，與 email 無關、非角色（一般使用者）看不到、不做商家驗證消不掉；(ii) **email-first 對 OAuth 無法強制指定帳號**——輸入的 email 只決定要顯示哪張 continue 頁、不會傳給 provider，故 Google 會跳選擇器、FB 會用瀏覽器當下登入的帳號（**非安全漏洞**：永遠只登入自己 session 的帳號、騙不到別人，最多是顯示 email 與實際帳號不一致的 UX 困惑）。可選改法：Google 端改用 `login_hint`（移除全域 `prompt=select_account`、改逐連結帶 `auth_params`）、FB 無解；作者決定維持現狀。**
- **下一步**：items 5–10 與**前台 UI/UX 改造（首頁登入 landing + 儀表板 modal 建立／點擊複製／可展開明細 + 登入卡版面微調）皆已部署上 Cloud Run 並真人驗收通過、全部上線**（線上 `/livez` 回 ok；部署用 README 部署章節新加的「重新部署」runbook，內含 `shortlink-499808` 正確專案 ID 與「只給 `--image`、勿帶 `--set-secrets`」的眉角）。**Stage 4 — 效能（作者指定優先項）進行中**：**優化 1（重導 Redis cache-aside）已實作、本機驗證、並部署上線真人驗收通過**（revision `shortlink-00007-gt7`；prod 接 **Upstash** serverless Redis〔GCP 東京 `asia-northeast1`，free tier〕，URL 走 Secret Manager `redis-url`；`prod.py` 已設成缺 `REDIS_URL` 即啟動報錯，Dockerfile 的 build-time `collectstatic` 加了佔位 `REDIS_URL`；Upstash 後台確認線上走真 Redis〔Writes=建立的 DEL＋miss 的 SET、Reads=GET＋連線 handshake〕）。剩 DB 索引 + `EXPLAIN` 分析、非同步寫入點擊、建立/重導加 rate limiting。協作方式改為「作者本人寫所有程式碼、Claude 只逐步指導」（文件 ADR/CLAUDE.md 由 Claude 代筆）。**儀表板「多選項排序下拉 + 編輯短網址」(h)(i) 與隱私權頁 (j) 已部署上線（revision `shortlink-00010-49m`，線上 `/livez`、`/privacy/` 驗證 OK）；Facebook 因 `email` 卡商家驗證，改用「測試者」名單放行 (k)、App 維持開發模式。** **Logo 已完成、OAuth 對外開放決策已拍板**（Google 同意畫面已發布 In production＝任何人可用 Google 登入；Facebook 因 Meta 要求商家驗證才能 go Live，維持 email + 開發模式 + 測試者，見 [`docs/adr/0012-facebook-public-profile-only.md`](docs/adr/0012-facebook-public-profile-only.md)）。 （`email` 已加回 FB use case ✅、測試者登入恢復取得 email）待辦：README 補一句「FB 為開發模式、任意人請用 Google 登入、需 FB 測試可來信加測試者」；以及 Stage 4 剩餘項（DB 索引 + `EXPLAIN`、非同步寫入點擊、rate limiting、Gunicorn workers、`migrate` 改 Cloud Run Job、`my_links` 分頁）。
...
- [ ] **`my_links` 儀表板分頁**：目前 `request.user.short_links` 沒有任何筆數上限，使用者連結數一旦變大（例如百萬筆級別）會一次撈出全部資料、效能變差。先用 Django 內建 `Paginator`（offset-based）做傳統分頁；若之後資料量規模需要更穩定的效能，再考慮 cursor-based（無限滾動式）分頁。
