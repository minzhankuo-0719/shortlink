...
- **下一步**：items 5–10 已部署上 Cloud Run 並真人驗收通過（登入/UX 打磨全部上線）。**Stage 4 — 效能（作者指定優先項）進行中**：**優化 1（重導 Redis cache-aside）已由作者親手實作並本機驗證通過（尚未部署）**；剩 DB 索引 + `EXPLAIN` 分析、非同步寫入點擊、建立/重導加 rate limiting。協作方式改為「作者本人寫所有程式碼、Claude 只逐步指導」（文件 ADR/CLAUDE.md 由 Claude 代筆）。正式環境 Redis 接哪裡（Upstash／Memorystore）待部署時再定。
