...
### D2. Django 正式環境（`DEBUG=False`）發生例外，要去哪裡看錯誤？

**簡答**
Django 的預設 `LOGGING` 設定裡，「印到 console」的 handler 只在 `DEBUG=True` 時生效；`DEBUG=False` 時改成用 `mail_admins`（寄信給 `ADMINS`），但如果沒設定 `ADMINS`，這個 handler 會直接靜默跳過——等於例外完全沒有任何輸出，不是「日誌系統壞了」，是 Django 預設行為本來就這樣設計。這個專案在 [`config/settings/prod.py`](../config/settings/prod.py) 額外加了一段 `LOGGING`，把 root logger 接上一個單純的 console handler，讓任何等級 `INFO` 以上的訊息（包含未處理例外的完整 traceback）都印到 stderr。

**延伸**
- Cloud Run（以及大多數容器平台）會自動把容器的 stdout/stderr 收進它自己的日誌系統（這裡是 Cloud Logging），所以只要確保 Django 把東西印到 console，不需要額外裝 Sentry 之類的第三方錯誤追蹤服務就能看到錯誤——對這個規模的專案，裝額外服務是過度設計。
- 這個坑是怎麼發現的：把健康檢查端點從 `healthz` 改名成 `livez` 時漏改了 `home.html` 模板裡的 `{% url 'core:healthz' %}`，造成首頁 500，但一開始翻 Cloud Logging 完全找不到 traceback，加上這段 `LOGGING` 之後才看到真正原因是 `NoReverseMatch`。
- 延伸問題：為什麼不論 `DEBUG` 是 `True` 或 `False`，都不應該讓使用者在瀏覽器上看到完整的 Python traceback？（正式環境洩漏程式碼路徑、套件版本、甚至環境變數內容，是資安風險——這也是為什麼 Django 在 `DEBUG=False` 時改成靜默 + 寄信通知開發者，而不是直接印給使用者看。）

### D3. 為什麼 Cloud Run 的服務會有兩個網址？

**簡答**
Cloud Run 最近把預設網域的命名格式從 `<service>-<專案編號>.<region>.run.app`（舊格式）換成 `<service>-<隨機雜湊>-<地區代碼>.a.run.app`（新格式）。為了不讓舊的書籤/整合失效，**新舊兩個網址會同時、永久生效**，都指向同一個 Cloud Run 服務。`gcloud run deploy` 指令收尾印出的 `Service URL` 固定是舊格式，但 `gcloud run services describe --format='value(status.url)'` 查到的才是 Google 目前認定的正式網址。

**延伸**
- 這造成一個實際的坑：OAuth 的 redirect URI、Django 的 `CSRF_TRUSTED_ORIGINS` 如果只登記了其中一個網址，使用者用另一個網址訪問並嘗試登入，會收到 `redirect_uri_mismatch`——不是設定打錯，是兩個網址裡只有一個被登記過。
- 解法不是把兩邊都登記（這會讓服務有兩個「正式」入口，狀態管理變複雜），而是**固定只對外公開、分享其中一個網址**，這個專案選擇新格式那一個（`gcloud describe` 查到的那個）。
- 部署後第一件事該做：用 `gcloud run services describe --format='value(status.url)'` 查出 Google 自己認定的正式網址，所有後續設定都用這個值，不要直接複製貼上部署指令收尾印出的那行文字。
...
### F5. 怎麼讓 Google 登入每次都跳「選擇帳號」？為什麼 Facebook 做不到？

**簡答**
靠 OAuth 授權端點的參數 `prompt=select_account`，透過 allauth 的 `SOCIALACCOUNT_PROVIDERS["google"]["AUTH_PARAMS"]` 傳給 Google（`"AUTH_PARAMS": {"prompt": "select_account"}`）。`AUTH_PARAMS` 裡的東西不是 allauth 自己用的，而是它原封不動轉成查詢參數接到授權網址後面、交給 Google 處理。要注意 `AUTH_PARAMS` 必須是 **dict**（一組組「鍵=值」），寫成字串會壞。Facebook 沒有等價參數——它的 `AUTH_PARAMS` 只吃 `auth_type`，而 `auth_type=reauthenticate` 的語意是「**重新輸入密碼確認本人**」而非「換帳號」，所以 FB 想換帳號只能靠無痕視窗或先登出，這是平台限制不是設定問題。

**延伸**
- 一個有用的分類：`SCOPE`＝跟供應商要什麼資料、`AUTH_PARAMS`＝對授權端點下什麼指令、`APPS`＝你是哪個 OAuth app（client_id/secret）。遇到「想調整 OAuth 行為」先判斷「這是不是供應商原生支援的授權參數」，是的話多半走 `AUTH_PARAMS`，而不是去 allauth 找開關。
- `prompt` 是 OpenID Connect 定義的標準參數（`none`/`login`/`consent`/`select_account`），Google 支援；不同供應商支援的子集不同，這也是「OAuth 是協定、各家實作有差異」的具體體現。

### F6. 怎麼讓密碼重設頁只在正式環境失效、開發環境仍可用？

**簡答**
正式環境沒有 SMTP，allauth 的密碼重設一定寄不出信、直接 500；但開發環境設了 console email backend，本機還想留著它能用。做法分兩層：(1) 在分層 settings 加一個有語意的布林開關 `PASSWORD_RESET_ENABLED`（`base.py`=True 當常態預設、`prod.py`=False 覆寫、`dev.py` 不寫直接繼承 base 的 True）；(2) 在 `config/urls.py` 的 `include("allauth.urls")` **之前**，當開關關閉時用一個 `raise Http404` 的小 view 覆寫掉重設入口 `accounts/password/reset/` 與信件 key 確認頁 `accounts/password/reset/key/...`。Django URL 由上往下、先比中先贏，宣告在前面的覆寫就會搶先命中。

**延伸**
- 實作用「空 list + `*` 解包」讓開關乾淨控制：`_password_reset_overrides` 在 dev 是 `[]`，`*[]` 攤平後等於沒東西，dev 完全不受影響；prod 才填入兩條覆寫。擋或不擋只由「list 空不空」決定，而那由開關決定，一條邏輯線到底。
- 為什麼用「設定開關」而不是在 urls 裡硬寫 `if DEBUG`：開關有名字、有註解，意圖一眼就懂，且未來多一個環境（如有 SMTP 的 staging）只要決定那個布林即可，不必回頭改 URL 邏輯。
- 兩個 `.../done/` 純文字確認頁不擋——入口被擋就走不到，擋了只是雜訊（縱深防禦的取捨：擋「功能性」的兩條就夠表達「prod 重設關閉」這個意圖）。
- 驗證可不必真的部署：用 Django test client 配合臨時環境變數模擬兩種設定。測 prod 要 `secure=True`（避開 `SECURE_SSL_REDIRECT` 的 301）、要把 test client 的預設 Host `testserver` 加進 `ALLOWED_HOSTS`。
